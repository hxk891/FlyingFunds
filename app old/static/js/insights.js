// ============================================================================
// insights.js — FlyingFunds Intelligence Layer v2
// Health score, plain-English summaries, what-if engine, behavioural insights.
// All scoring driven by REAL risk metrics (Sharpe, VaR, drawdown, volatility).
...
// Returns { score, grade, colour, reasons, suggestions }
// All dimensions are driven by real metrics when available.
function computeHealthScore(metrics, holdings) {
  ...
  const hasRet     = _has(metrics?.annualised_return);
  const hasVaR     = _has(metrics?.var);
  const hasData    = hasSharpe || hasDD || hasVol;

  // ── Dimension 1: Diversification (max 30 pts) ────────────────────────────
  let divPts = 30;
  ...
  // ── Dimension 2: Sharpe ratio (max 25 pts) ───────────────────────────────
  if (hasSharpe) {
    ...
    else if (s < 0.6){ pts = 12; reasons.push({ type: 'info', text: `Sharpe ${_f2(s)} — below average. There is room to improve risk-adjusted returns.` }); suggestions.push(`A Sharpe of ${_f2(s)} means risk isn't being rewarded well. Reducing volatility through diversification would push this higher.`); }
    ...
    reasons.push({ type: 'info', text: 'Upload a price CSV in Settings to unlock Sharpe ratio, VaR, and drawdown scoring.' });
    suggestions.push('Go to Settings → Data & Export → Upload Price CSV to enable full risk metric scoring.');
    score -= 15; // partial deduction for missing data
  }

  // ── Dimension 3: Max drawdown (max 25 pts) ───────────────────────────────
  if (hasDD) {
    const dd = _abs(metrics.max_drawdown);
    const rec = dd > 0 ? ((1 / (1 - dd) - 1) * 100).toFixed(0) : 0;
    let pts = 25;
    if (dd > 0.5)       { pts = 0;  reasons.push({ type: 'bad',  text: `Max drawdown ${_pct(metrics.max_drawdown)} — severe. Recovery needs +${rec}%.` }); suggestions.push(`A ${_pct(metrics.max_drawdown)} drawdown is severe. Reducing position sizes and improving diversification would lower your worst-case loss.`); }
    else if (dd > 0.35) { pts = 8;  reasons.push({ type: 'warn', text: `Max drawdown ${_pct(metrics.max_drawdown)} — high. Recovery needs +${rec}%.` }); suggestions.push(`Your max drawdown of ${_pct(metrics.max_drawdown)} is high. Consider defensive positions or reducing exposure to high-beta holdings.`); }
    else if (dd > 0.2)  { pts = 15; reasons.push({ type: 'info', text: `Max drawdown ${_pct(metrics.max_drawdown)} — moderate. Recovery needs +${rec}%.` }); }
    else if (dd > 0.1)  { pts = 20; reasons.push({ type: 'good', text: `Max drawdown ${_pct(metrics.max_drawdown)} — manageable.` }); }
    else                { pts = 25; reasons.push({ type: 'good', text: `Max drawdown ${_pct(metrics.max_drawdown)} — very low. Portfolio has been resilient.` }); }
    score -= (25 - pts);
  }

  // ── Dimension 4: Volatility (max 20 pts) ─────────────────────────────────
  if (hasVol && hasRet) {
    ...
    if (ret < 0 && hasRet) {
      ...
      score -= 5;
    } else if (ret < 0.04 && hasRet && vol > 0.15) {
...
// portfolio summary ──────────────────────────────────────────
function generatePortfolioSummary(metrics, holdings, health, name) {
  ...
  let lines = [`**${name || 'Your portfolio'}** scores **${score}/100 (${grade})** — it ${gradeText}.`];
  ...
  // Performance narrative (only if real data available)
  if (hasMets) {
    ...
    // Return assessment
    if (ret < 0)       lines.push(`The portfolio has a negative annualised return of ${ret.toFixed(1)}%/yr — it is losing value over time.`);
    ...
    else if (ret < 12) lines.push(`Annualised return is ${ret.toFixed(1)}%/yr — solid performance broadly in line with equity market expectations.`);
    ...
    lines.push(`Upload a historical price CSV in Settings → Data & Export to unlock full risk metrics: Sharpe ratio, VaR, max drawdown, and volatility scoring.`);
  }
...
function computeWhatIf(scenario, metrics, holdings) {
  ...
  if (scenario === 'market_drop_10' || scenario === 'market_drop_20' || scenario === 'market_drop_30') {
    const pct   = scenario === 'market_drop_10' ? 0.10 : scenario === 'market_drop_20' ? 0.20 : 0.30;
    ...
    return {
      ...
      changes: [
        ...
        { label: 'Max drawdown',      before: _pct(dd),    after: (newDD*100).toFixed(1)+'%', delta: `−${(pct*100).toFixed(0)}pp`, bad: true },
      ],
      insight: `A ${label} market fall would cost approximately £${loss.toLocaleString('en-GB',{minimumFractionDigits:2})}. ${_abs(dd) < pct ? `This would be your portfolio's worst-ever drawdown — recovery would require a +${rec}% gain.` : `Your portfolio has already survived a worse drawdown of ${(_abs(dd)*100).toFixed(1)}%, so this is within its historical range.`} The critical question: would you hold through this decline without selling? Selling at the bottom locks in losses permanently and means missing the recovery.`
    };
  }
  ...
  if (scenario === 'rebalance_equal') {
    ...
    return {
      scenario: 'If you equal-weighted all holdings',
  ...
  if (scenario === 'add_5pct_cash') {
    ...
    return {
      scenario: 'If you invested an additional 5%',
      ...
      changes: [
        ...
        { label: 'Risk metrics', before: 'Unchanged', after: 'Unchanged', delta: '—', bad: false },
        { label: 'Est. value (10yr)', before: '—', after: `£${grow10.toLocaleString('en-GB',{minimumFractionDigits:2})}`, delta: 'compounded', bad: false },
      ],
...
function analyseBehaviour(trades, holdings, metrics) {
  ...
  // 1. Overtrading
  const now    = Date.now();
  const _30d   = now - 30  * 86400000;
  const _7d    = now - 7   * 86400000;
  const recent = trades.filter(t => new Date(t.ts).getTime() > _30d);
  const week   = trades.filter(t => new Date(t.ts).getTime() > _7d);

  if (recent.length > 15) {
    insights.push({ type:'warn', icon:'⚡', title:'High trading frequency',
      text:`${recent.length} trades in the last 30 days. Research shows overtrading reduces returns by 1–2%/yr due to costs and poor timing. Ask yourself: were these trades driven by analysis or emotion?`, bias:'Overconfidence bias' });
  } else if (week.length > 5) {
    insights.push({ type:'info', icon:'📊', title:'Active week of trading',
      text:`${week.length} trades this week. Frequent trading isn't bad if driven by analysis — but check whether each trade had a pre-written thesis.`, bias:'Possible overconfidence' });
  }

  // 2. Concentration risk (behavioural signal)
  if (holdings?.length > 0) {
    const maxW  = Math.max(...holdings.map(h => Number(h.weight)||0));
    const topH  = holdings.find(h => Number(h.weight) === maxW);
    if (maxW > 0.45) {
      insights.push({ type:'warn', icon:'🎯', title:`Heavy concentration: ${topH?.symbol||'one stock'} is ${_pct(maxW)}`,
        text:`${_pct(maxW)} in a single position may reflect overconfidence in that stock or an anchoring effect (reluctance to reduce a past winner). MPT shows this adds uncompensated idiosyncratic risk.`, bias:'Overconfidence / anchoring' });
    } else if (maxW > 0.28) {
      insights.push({ type:'info', icon:'⚖️', title:`${topH?.symbol||'Largest holding'} is ${_pct(maxW)} of portfolio`,
        text:`Above the 20–25% guideline. This may be intentional conviction or simply that the position has grown and you haven't rebalanced — worth reviewing.`, bias:'Status quo bias' });
    }
  }

  // 3. Performance-based behavioural signal
  if (_has(metrics?.sharpe_ratio)) {
    const s = Number(metrics.sharpe_ratio);
    if (s < 0) {
      insights.push({ type:'bad', icon:'📉', title:'Negative risk-adjusted return',
        text:'Your Sharpe ratio is negative — you are bearing risk without being rewarded. This pattern can cause investors to double down on losing positions hoping for a recovery, rather than rationally reassessing each holding.', bias:'Loss aversion / sunk cost' });
    }
  }
  ...
  if (sells.length >= 2 && _has(metrics?.max_drawdown) && _abs(metrics.max_drawdown) > 0.18) {
    insights.push({ type:'info', icon:'😰', title:'Selling during drawdown period',
      text:`Your portfolio experienced a ${(_abs(metrics.max_drawdown)*100).toFixed(1)}% drawdown. Selling during market declines locks in losses and typically means missing the recovery — the most costly outcome in investing. Review whether sell decisions were driven by strategy or discomfort.`, bias:'Loss aversion' });
  }
  ...
  if (insights.length === 0) {
    if (trades.length === 0) {
      insights.push({ type:'good', icon:'✅', title:'No patterns yet',
        text:'Start paper trading to build a trade history — Halo will analyse for overtrading, loss aversion, and FOMO patterns over time.', bias: null });
    } else {
      insights.push({ type:'good', icon:'✅', title:'No major behavioural red flags',
        text:'Trading activity looks measured. Keep logging decisions in the Investment Diary — it reveals subtle bias patterns that numbers alone miss.', bias: null });
    }
  }
  ...
  el.innerHTML = `
    <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap;">
      <div style="text-align:center;flex-shrink:0;">
        ...
        <div style="margin-top:5px;font-size:20px;font-weight:700;color:${colour};font-family:'DM Sans',sans-serif;">${grade}</div>
      </div>
      <div style="flex:1;min-width:160px;">
        ${show.map(r => {
          ...
          return `<div style="display:flex;gap:7px;align-items:flex-start;margin-bottom:6px;">
            ...
            <span style="font-size:12px;color:#2d1f1a;font-family:'DM Sans',sans-serif;line-height:1.5;">${r.text}</span>
          </div>`;
        }).join('')}
        ${suggestions.length ? `
          <div style="margin-top:6px;padding:8px 10px;background:#fff;border-radius:8px;border-left:3px solid ${colour};">
            <span style="font-size:11px;font-weight:700;color:${colour};font-family:'DM Sans',sans-serif;text-transform:uppercase;letter-spacing:0.06em;">Improve: </span>
            <span style="font-size:11px;color:#555;font-family:'DM Sans',sans-serif;line-height:1.5;">${suggestions[0]}</span>
          </div>` : ''}
      </div>
    </div>`;
}
...
function renderWhatIf(containerId, result) {
  ...
  el.innerHTML = `
    <div style="font-family:'DM Sans',sans-serif;">
      ...
      <div style="font-size:11px;color:#888;margin-bottom:12px;line-height:1.5;">${result.assumption}</div>
      <table style="width:100%;border-collapse:collapse;margin-bottom:12px;">
        <thead><tr style="border-bottom:1.5px solid #e5e5e5;">
          <th style="text-align:left;font-size:11px;color:#888;font-weight:500;padding:3px 0;">Metric</th>
          <th style="text-align:right;font-size:11px;color:#888;font-weight:500;padding:3px 6px;">Before</th>
          <th style="text-align:right;font-size:11px;color:#888;font-weight:500;padding:3px 6px;">After</th>
          <th style="text-align:right;font-size:11px;color:#888;font-weight:500;padding:3px 0;">Change</th>
        </tr></thead>
        <tbody>
          ${result.changes.map(c => `<tr style="border-bottom:1px solid #f0f0f0;">
...
// ── Tooltip: attach explainable tooltip to a metric element ───────────────────
