...
def pnl(portfolio_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    # Fetch all unique symbols in parallel — one HTTP call per unique symbol,
    # not one per holding. Cache means repeat calls within 60s are free.
...
# ── Investment chart (personal holding value over time) ───────────────────────
...
def investment_chart(
    ...
    # ── Strategy 1: yfinance historical candles (free) ───────────────────────
    candle_dates  = []
    candle_closes = []
    ...
    # ── Strategy 2: Our own prices table ────────────────────────────────────
    if not candle_dates:
    ...
    # ── Build series from historical prices (Strategy 1/2) ──────────────────
    # Walk through each price day; apply trades to get running qty, then
    # multiply qty × close to get the holding value on that day.
    series = []
    source = "trades_only"
    ...
    if candle_dates and candle_closes:
        ...
        # ── Strategy 3: Trade-point-only (no price history available) ────────
        running_qty = 0.0
        cost_basis  = 0.0
        for t in trades:
            if t.side == "BUY":
        ...
        # Append today's live price as the final point
        try:
            api_key = os.getenv("FINNHUB_API_KEY", "")
