"""
News feed is using free RSS sources hence no API key required.
Multiple feeds fetched in parallel, deduplicated and cached 15 minutes.
"""
import time
import feedparser
...
# server-side cache (15 min mark)
_cache: dict = {"articles": [], "ts": 0}
_cache_lock = Lock()
CACHE_TTL = 15 * 60

#parse the feed and return list of articles. 
def _parse_feed(feed_meta: dict) -> list:
    try:
        ...
        for entry in parsed.entries[:8]:
            ...
            # tyr implementing images media_thumbnail, media_content, enclosures
            image = None
...
# routes
#articles are loaded client-side by /api/news."""
